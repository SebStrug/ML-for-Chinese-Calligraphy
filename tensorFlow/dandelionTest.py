...
#  tf.contrib.tensorboard.plugins.projector.visualize_embeddings(writer, config)
  ...
  while i<iterations:
      ...
      if i % (displayNum) == 0:
          ...
          [train_accuracy, s] = sess.run([accuracy, summ], feed_dict={x: batchImages, y: batchLabels})
          writer.add_summary(s, i)
          #train_accuracy = accuracy.eval(feed_dict={x: batchImages, y_: batchLabels, keep_prob: 1.0})
      if i%(testNum) == 0 and i!=0:
          print("evaluating test accuracy...")
          sess.run(assignment, feed_dict={x: testImages[:1024], y: testLabels[:1024]})
      ...
      sess.run(train_step, feed_dict={x: batchImages, y: batchLabels})
