...
def discriminator_image(x, isTrain=True, reuse=False): #reuse=True by default?
    with tf.variable_scope('discriminator_image', reuse=reuse): #tf.AUTO_REUSE instead of reuse?
        print("Running discriminator which takes in an image...")
...
def discriminator_latent(z, isTrain=True, reuse=False):
     with tf.variable_scope('discriminator_latent', reuse=reuse): 
         print("Running discriminator which takes in latent representations...")
         conv1 = tf.layers.conv2d(z, 512, [1, 1], strides=(1, 1), padding='same')
         print("Conv1 shape: {}".format(conv1.get_shape()))
         lrelu1 = tf.nn.leaky_relu(conv1)
...
def discriminator_tuple(x,z, isTrain=True, reuse=False):
    with tf.variable_scope('discriminator_tuple', reuse=reuse): 
        ...
        conv2 = tf.layers.conv2d(lrelu1,128, [1,1], strides = (1,1), padding='same')
        print("Conv2 shape: {}".format(conv2.get_shape()))
...
def generator_image(z, isTrain=True, reuse=False):
    with tf.variable_scope('generator_x', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d_transpose(z, 1024, [3, 3], strides=(1, 1), padding='valid')
        print("Conv1 shape: {}".format(conv1.get_shape()))
        ...
        print("Conv5 shape: {}".format(conv5.get_shape()))
