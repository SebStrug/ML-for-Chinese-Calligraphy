...
class machineLearning(object):
    ...
    def createZippedList(characters,name):
        ...
        fF.saveNPZ(name,saveChars=[i[0] for i in zippedList],saveIndex=[i[1] for i in zippedList])
        return zippedList
    
    def storeCharNumber(character,name):
        listCharacters,index = fF.readNPZ(name,'saveChars','saveIndex')
        return np.where(listCharacters==ord(character))[0][0]
        
    def newHotOnes(characters,name):
        characters = [ord(i) for i in characters] #convert to numbers
        zippedList = machineLearning.createZippedList(characters,name)
